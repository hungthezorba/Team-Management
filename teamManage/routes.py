		...
		for member in membersAdd:
			if not (member in member_in_team):  # a condition to check if member is already in the team or not
				user = User.query.filter_by(
					username=member).first()  # those line will ensure that the DB wont have any duplicated rows
...
@socketio.on('my handleMember', namespace='/chat')
def handle_event_member(json, methods=['GET', 'POST']):
	team_id = request.args.get('team_id')
	while team_id == None:
		team_id = request.args.get('team_id')
	emit('my members', json, callback=messageReceived(), room=int(team_id))
